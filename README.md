...
とあったときに、左辺と右辺の値を受け取って1つの値を返す関数です。
...
この場合、A+BとC+Dの間に!演算子がありますが、!演算子は右の値と結合するので、二つの独立した処理として扱えます。
...
上では、左辺の名前に右辺の値を割り当てるようなunite演算子=を定義しています。
...
しかし、システムは変数が見つからなくてもすぐにエラーを出すことはなく、name=3が書かれているスコープに、nameを不定型に束縛します。

=の引数に値を渡すときに、不定型のnameは不定型ではなく、名前がnameな名前型を渡します。

処理の終了時に不定型が残っている場合、エラーを出します。
